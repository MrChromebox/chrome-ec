...
static int vivaldi_apply_top_row_mode(enum keybd_top_row_mode mode)
{
	...
	for (i = 0; i < ARRAY_SIZE(vivaldi_keys); i++) {
		...
		*mask |= BIT(row);
		...
		/*
		 * Recovery hotkey (Alt+VolUp+R) needs the vol-up position.
		 * Only bind it while emitting action codes.
		 */
		if (mode == KEYBD_TOP_ROW_ACTION && key == TK_VOL_UP)
...
static enum ec_status
keybd_top_row_command(struct host_cmd_handler_args *args)
{
...
static int command_toprow(int argc, const char **argv)
{
